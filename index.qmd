...
sample_overlap <- "Bias due to sample overlap"
...
dat %>%
  ...
  select(`Method`, `Code repository (URL)`, `Analysis Type`, `Data Types`, `Robust To`) %>%
  datatable(escape = FALSE, filter = "top", extensions = c('ColReorder', 'Buttons'), 
            options = list(colReorder = TRUE, dom = "Bfrtip", 
                           buttons = I('colvis'), 
                           columnDefs = list(list(visible = FALSE, targets = c(2,4)))),
