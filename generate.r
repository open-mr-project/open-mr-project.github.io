...
dat$`Abbreviated method name`[is.na(dat$`Abbreviated method name`)] <-
  dat$`Method name`[is.na(dat$`Abbreviated method name`)]
dat$title <- dat$`Abbreviated method name`
dat$author <- dat$`Primary contact`
dat$`Publication DOI` <- fix_doi(dat$`Publication DOI`)
dat$pubdate <-
  sapply(dat$`Publication DOI`, \(x) Diderot::get_date_from_doi(x, TRUE))
...
metadata <- tibble(names = names(dat),
